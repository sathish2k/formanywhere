...
import type {
  ...
  RuleAction,
  DroppedElement,
} from '../form-builder.configuration';
...
export class RulesEngine {
  ...
  private evaluateCondition(condition: Condition): boolean {
    ...
    switch (condition.operator) {
      ...
      case 'isNotChecked':
        return (
          !fieldValue || fieldValue === false || fieldValue === 'false' || fieldValue === 0
        );
      ...
      if (element.validation) {
        if (element.validation.minLength && value.length < element.validation.minLength) {
          errors[element.id] =
            ...
            `Minimum length is ${element.validation.minLength}`;
          isValid = false;
          return;
        }
        ...
        if (element.validation.maxLength && value.length > element.validation.maxLength) {
          errors[element.id] =
            element.validation.customMessage ||
            `Maximum length is ${element.validation.maxLength}`;
          isValid = false;
          return;
        }
        ...
        if (element.validation.pattern) {
          ...
          if (!regex.test(value)) {
            errors[element.id] = element.validation.customMessage || 'Invalid format';
            isValid = false;
            return;
          }
        }
        ...
        if (element.validation.min !== undefined && Number(value) < element.validation.min) {
          errors[element.id] =
            element.validation.customMessage || `Minimum value is ${element.validation.min}`;
          isValid = false;
          return;
        }
        ...
        if (element.validation.max !== undefined && Number(value) > element.validation.max) {
          errors[element.id] =
            element.validation.customMessage || `Maximum value is ${element.validation.max}`;
          isValid = false;
          return;
        }

        if (element.validation.customRule && !element.validation.customRule(value)) {
          errors[element.id] = element.validation.customMessage || 'Invalid value';
          isValid = false;
          return;
        }
      }
