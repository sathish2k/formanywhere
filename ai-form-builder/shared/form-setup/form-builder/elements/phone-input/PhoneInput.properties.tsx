...
import {
  type PhoneInputPropertiesFormData,
  phoneInputPropertiesSchema,
} from './phone-input.schema';

interface PhoneInputPropertiesProps {
  element: DroppedElement;
  onUpdate: (updates: Partial<DroppedElement>) => void;
}

export function PhoneInputProperties({ element, onUpdate }: PhoneInputPropertiesProps) {
  const { control, watch, reset, register } = useForm<PhoneInputPropertiesFormData>({
    resolver: zodResolver(phoneInputPropertiesSchema),
    defaultValues: {
      label: element.label || '',
