...
import {
  type TimePickerPropertiesFormData,
  timePickerPropertiesSchema,
} from './time-picker.schema';

interface TimePickerPropertiesProps {
  element: DroppedElement;
  onUpdate: (updates: Partial<DroppedElement>) => void;
}

export function TimePickerProperties({ element, onUpdate }: TimePickerPropertiesProps) {
  const { control, watch, reset, register } = useForm<TimePickerPropertiesFormData>({
    resolver: zodResolver(timePickerPropertiesSchema),
    defaultValues: {
      label: element.label || '',
